...
- ✅ Совместный доступ: кнопка «Поделиться» генерирует уникальный токен и копирует ссылку вида `/shared?token=…`. Открыв её в новом окне, любой пользователь увидит список без запроса пароля и сможет только отмечать товары как выполненные или отсутствующие, а также перемещать группы.

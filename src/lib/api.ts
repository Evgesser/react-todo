...
// --- lists ---------------------------------------------------------------
export async function fetchLists(password: string): Promise<List[]> {
  const res = await fetch(`${BASE}/lists?password=${encodeURIComponent(password)}`);
...
export async function createList(password: string, name: string, defaultColor: string) {
  const res = await fetch(`${BASE}/lists`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ password, name, defaultColor }),
  });
  return res.json();
}
...
export async function fetchPersonalization(password: string): Promise<{
  categories?: StoredCategory[];
  templates?: Template[];
}> {
  const res = await fetch(`${BASE}/personalization?password=${encodeURIComponent(password)}`);
